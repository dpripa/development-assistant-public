...
function initActivationManager() {
  if ('no' === window.wp_dev_assist_plugins_screen.has_deactivated_plugins) {
    return;
  }
  var $btnActivate = $('<a/>');
  $btnActivate.addClass('button').attr('href', window.wp_dev_assist_plugins_screen.plugin_activation_url).text(window.wp_dev_assist_plugins_screen.plugin_activation_title);
  $('.bulkactions').after($btnActivate);
  if ('no' === window.wp_dev_assist_plugins_screen.reset) {
    return;
  }
  $('#deactivate-development-assistant').on('click', function (event) {
    event.preventDefault();
    window.location.href = window.wp_dev_assist_plugins_screen.plugin_activation_url + '&' + window.wp_dev_assist_plugins_screen.deactivation_reset_query_key + '=yes';
  });
}
/* WEBPACK VAR INJECTION */}.call(this, __webpack_require__(/*! jquery */ "jquery")))
